...
const WEBHOOK_URL =
  (process as any).env.WEBHOOK_URL || "http://localhost:4000/webhook";
...
const broadcastState = (gameId: string) => {
  ...
  if (!g) return;
  const players = Object.entries(g.players).map(([id, p]) => ({
    id,
    name: p.name,
    x: p.x,
    y: p.y,
...
io.on("connection", (socket) => {
  ...
  socket.on(
    "join",
    async (msg: { gameId: string; name: string; walletAddress: string }) => {
      ...
      if (Object.keys(g.food).length === 0) {
        try {
          ...
          // Distribute totalDeposits across 20 food items
          const depositPerFood = Number(totalDeposits) / 20;
          Array.from({ length: 20 }, () => {
            const id = genId();
            g.food[id] = {
              id,
              ...randPos(),
              score: Math.floor(depositPerFood / 1e18), // Convert from wei to ether
            };
          });
        } catch (error) {
          console.error("Failed to fetch totalDeposits:", error);
          // Fallback to random scores if contract read fails
          Array.from({ length: 20 }, () => {
            const id = genId();
            g.food[id] = { id, ...randPos(), score: randScore() };
          });
        }
      }
      socket.join(gameId);
      ...
      console.log(
        `[JOIN] ${msg.name} joined game ${gameId} as ${playerId} (${msg.walletAddress})`
      );
    }
  );
  ...
  socket.on("move", (msg: { x: number; y: number }) => {
    ...
    Object.values(g.food).forEach((food) => {
      if (
        dist(player, food) <
        playerRadius(player.score) + foodRadius(food.score)
      ) {
        player.score += food.score;
        handleFoodEat(food.score, player.walletAddress);
        fetch(WEBHOOK_URL, {
          method: "POST",
          ...
          .then(() =>
            console.log(
              `[WEBHOOK] Sent for ${player.name} eating food ${food.id} in ${gameId}`
            )
          )
          .catch(() =>
            console.log(
              `[WEBHOOK] Failed for ${player.name} eating food ${food.id} in ${gameId}`
            )
          );
      ...
      if (otherId !== playerId) {
        ...
        if (dist(player, other) < r1 + r2) {
          if (player.score > other.score) {
            ...
            console.log(
              `[PLAYER EAT] ${player.name} (${playerId}) ate ${other.name} (${otherId}) and gained ${other.score} points in ${gameId}`
            );
            ...
            broadcastState(gameId!);
            console.log(
              `[PLAYER EAT] ${other.name} (${otherId}) ate ${player.name} (${playerId}) and gained ${player.score} points in ${gameId}`
            );
            return;
          }
        }
    ...
    broadcastState(gameId!);
    console.log(
      `[MOVE] ${player.name} (${playerId}) moved to (${msg.x},${msg.y}) in ${gameId}`
    );
  });
  ...
  socket.on("disconnect", () => {
    ...
    if (g && g.players[playerId!]) {
      console.log(
        `[LEAVE] ${g.players[playerId!].name || playerId} left game ${gameId}`
      );
      delete g.players[playerId!];
      broadcastState(gameId!);
